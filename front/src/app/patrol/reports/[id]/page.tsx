...
export default function ReporterReportDetailPage() {
  const { id } = useParams<{ id: string }>(); const { userLevel } = useAuth(); const [report, setReport] = useState<PatrolReport | null>(null); const [history, setHistory] = useState<ReviewHistoryItem[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null);
  ...
  useEffect(() => { if (userLevel === "Patrol") void load(); }, [userLevel, load]);
  if (userLevel !== "Patrol") return <RoleNotice />; if (loading) return <div className="h-96 animate-pulse rounded-2xl bg-white/5" />; if (error || !report) return <div className="rounded-2xl border border-rose-400/20 bg-rose-400/10 p-8 text-center text-rose-100">{error || "گزارش یافت نشد."}<button onClick={() => void load()} className="mt-4 block mx-auto rounded-lg border border-white/10 px-4 py-2 text-sm">تلاش دوباره</button></div>;
