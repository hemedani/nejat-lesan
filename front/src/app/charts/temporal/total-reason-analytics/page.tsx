...
const TemporalTotalReasonAnalyticsPage = () => {
  ...
  const [chartData, setChartData] = useState<TemporalTotalReasonData | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [isDemoMode, setIsDemoMode] = useState(false);
  const [appliedFilters, setAppliedFilters] = useState<ChartFilterState>(DEFAULT_FILTER_STATE);
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.body?.analytics) {
        ...
        setActiveReasons(temporalData.series.length > 0 ? [temporalData.series[0].name] : []);
        setIsDemoMode(false);
      } else {
        console.warn("[TemporalTotalReasonAnalytics] API failed, using demo data:", response);
      ...
      console.error("[TemporalTotalReasonAnalytics] Network error, using demo data:", err);
      setChartData(DEMO_DATA);
      setActiveReasons([DEMO_DATA.series[0].name]);
      setIsDemoMode(true);
      setError(null);
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          series: analyticsData.series.map((series: { name: string; data: number[] }) => ({
            name: series.name || "نامشخص",
            data: Array.isArray(series.data) ? series.data : [],
          })),
        };
      } else {
        console.warn("[TemporalTotalReasonAnalytics] Invalid API response structure, using demo data");
        return DEMO_DATA;
      }
    } catch (error) {
      console.error("[TemporalTotalReasonAnalytics] Error transforming API data:", error);
      return DEMO_DATA;
    }
  };
  ...
  const handleApplyFilters = async (filters: ChartFilterState) => {
    ...
    try {
      ...
      if (response.success && response.body?.analytics) {
        ...
        console.warn("[TemporalTotalReasonAnalytics] Filter API failed, using demo data:", response);
        setChartData(DEMO_DATA);
        setActiveReasons([DEMO_DATA.series[0].name]);
        setAppliedFilters(filters);
        setIsDemoMode(true);
      ...
      console.warn("[TemporalTotalReasonAnalytics] Filter network error, using demo data:", err);
      setChartData(DEMO_DATA);
      setActiveReasons([DEMO_DATA.series[0].name]);
      setAppliedFilters(filters);
      setIsDemoMode(true);
        ...
        <div className="flex-1 p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">مقایسه زمانی علت تامه تصادفات</h1>
                <p className="text-sm text-gray-600 mt-1">
                  تحلیل روند زمانی ۱۰ علت برتر تصادفات و مقایسه آن‌ها در طول زمان
                </p>
              </div>
            ...
            {chartData && !isLoading && !isDemoMode && (
              <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-sm text-green-700">
                  تحلیل زمانی علت تامه تصادفات با {chartData.series.length} علت برتر شناسایی شده - دوره
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">{chartData.series.length}</div>
                    <div className="text-sm text-blue-800">تعداد علل شناسایی شده</div>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg">
                    <div className="text-2xl font-bold text-green-600">{activeReasons.length}</div>
