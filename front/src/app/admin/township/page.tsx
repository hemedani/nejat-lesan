import { TownshipDashboard } from "@/components/template";
import SearchBox from "@/components/molecules/SearchBox";
import Pagination from "@/components/molecules/Pagination";
import { ReqType } from "@/types/declarations/selectInp";
import { gets } from "@/app/actions/township/gets";
import { count } from "@/app/actions/township/count";
import { remove } from "@/app/actions/township/remove";
...
export default async function TownshipPage({
  ...
  searchParams: Promise<ReqType["main"]["township"]["gets"]["set"]>;
}) {
  const { limit = "20", page = "1", name } = await searchParams;

  const set: ReqType["main"]["township"]["gets"]["set"] = {
    limit: +limit || 20,
    page: +page,
    name,
  };
  const get: ReqType["main"]["township"]["gets"]["get"] = {
    _id: 1,
    name: 1,
  };
      ...
      <div className="flex items-start">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl text-gray-800 font-bold">
            {translateModelNameToPersian("township")}
          </h1>
          <p className="text-gray-500 mt-2 text-sm">
            مدیریت {translateModelNameToPersian("township")}
          </p>
        </div>
      </div>
