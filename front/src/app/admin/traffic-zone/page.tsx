...
import { gets } from "@/app/actions/traffic_zone/gets";
import { count } from "@/app/actions/traffic_zone/count";
import { remove } from "@/app/actions/traffic_zone/remove";
...
export default async function AirStatusDashboard({
  ...
  searchParams: Promise<ReqType["main"]["traffic_zone"]["gets"]["set"]>;
}) {
  const { limit = "20", page = "1", name } = await searchParams;

  const set: ReqType["main"]["traffic_zone"]["gets"]["set"] = {
    limit: +limit || 20,
    page: +page,
    name,
  };
  const get: ReqType["main"]["traffic_zone"]["gets"]["get"] = {
    _id: 1,
    name: 1,
  };
      ...
      <div className="flex items-start">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl text-gray-800 font-bold">
            {translateModelNameToPersian("traffic_zone")}
          </h1>
          <p className="text-gray-500 mt-2 text-sm">
            مدیریت {translateModelNameToPersian("traffic_zone")}
          </p>
        </div>
      </div>
