        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              نمایش قابلیت‌های مختلف کامپوننت فیلتر برای سیستم تحلیل ایمنی ترافیک
            </p>
          </div>
          <button
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                ...
                <span className={`text-sm font-bold ${
                  configPresets[activePreset].disableSeverityFilter ? 'text-red-600' : 'text-green-600'
                }`}>
                  {configPresets[activePreset].disableSeverityFilter ? '❌ غیرفعال' : '✅ فعال'}
                </span>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span className="text-sm">نوع برخورد:</span>
                <span className={`text-sm font-bold ${
                  configPresets[activePreset].disableCollisionTypeFilter ? 'text-red-600' : 'text-green-600'
                }`}>
                  {configPresets[activePreset].disableCollisionTypeFilter ? '❌ غیرفعال' : '✅ فعال'}
                </span>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span className="text-sm">روشنایی:</span>
                <span className={`text-sm font-bold ${
                  configPresets[activePreset].disableLightingFilter ? 'text-red-600' : 'text-green-600'
                }`}>
                  {configPresets[activePreset].disableLightingFilter ? '❌ غیرفعال' : '✅ فعال'}
                </span>
              </div>
              <div className="flex items-center justify-between p-2 bg-gray-50 rounded">
                <span className="text-sm">قفل شدید:</span>
                <span className={`text-sm font-bold ${
                  configPresets[activePreset].lockToSevereAccidents ? 'text-orange-600' : 'text-gray-600'
                }`}>
                  {configPresets[activePreset].lockToSevereAccidents ? '🔒 فعال' : '🔓 غیرفعال'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-8 h-full">
              <div className="text-center">
                ...
                <p className="text-gray-600 mb-6">
                  پیکربندی فعلی: <span className="font-semibold text-blue-600">{presetDescriptions[activePreset]}</span>
                </p>

                <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 inline-block">
                  <h3 className="font-semibold text-blue-900 mb-3">مشخصات پیکربندی</h3>
                  <div className="text-sm space-y-2">
                    <div className="flex justify-between items-center">
                      <span>فیلتر شدت:</span>
                      <span className={configPresets[activePreset].disableSeverityFilter ? 'text-red-600' : 'text-green-600'}>
                        {configPresets[activePreset].disableSeverityFilter ? '❌' : '✅'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>نوع برخورد:</span>
                      <span className={configPresets[activePreset].disableCollisionTypeFilter ? 'text-red-600' : 'text-green-600'}>
                        {configPresets[activePreset].disableCollisionTypeFilter ? '❌' : '✅'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>روشنایی:</span>
                      <span className={configPresets[activePreset].disableLightingFilter ? 'text-red-600' : 'text-green-600'}>
                        {configPresets[activePreset].disableLightingFilter ? '❌' : '✅'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span>قفل شدید:</span>
                      <span className={configPresets[activePreset].lockToSevereAccidents ? 'text-orange-600' : 'text-gray-600'}>
