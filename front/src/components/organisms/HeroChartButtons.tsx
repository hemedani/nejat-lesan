...
/**
 * Landing-page CTAs that enter module-owned surfaces (charts/maps).
 * Anonymous visitors always see them (marketing); logged-in users on an
 * install where the charts module is off get nothing that advertises it.
 */
export const HeroChartButtons = () => {
  const { isAuthenticated, hasModule } = useAuth();
  if (isAuthenticated && !hasModule("charts")) return null;
