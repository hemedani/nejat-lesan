...
const ChartNavigation: React.FC<ChartNavigationProps> = ({
    ...
    switch (section) {
      case 'overall':
        return [
          ...
          { id: 'hourly-day-of-week', label: 'تحلیل انواع برخورد', href: '/charts/overall/collision-analytics' }
        ]
      case 'temporal':
        return [
      ...
      case 'spatial':
        return [
          { id: 'regional', label: 'تحلیل منطقه‌ای', href: '/charts/spatial/regional' },
          { id: 'hotspots', label: 'نقاط داغ', href: '/charts/spatial/hotspots' }
        ]
      case 'trend':
        return [
          { id: 'monthly-trend', label: 'روند ماهانه', href: '/charts/trend/monthly-trend' },
          { id: 'yearly-trend', label: 'روند سالانه', href: '/charts/trend/yearly-trend' }
        ]
      default:
        return []
    }
  ...
  const generateBreadcrumbs = () => {
    ...
    if (pathname.includes('/charts')) {
      ...
      if (currentSection) {
        const sectionLabel = chartSections.find(s => s.id === currentSection)?.label || currentSection
        breadcrumbs.push({
          label: sectionLabel,
        ...
        if (currentChart) {
          const chartLabel = getChartNavigation(currentSection).find(c => c.id === currentChart)?.label || currentChart
          breadcrumbs.push({
            label: chartLabel,
