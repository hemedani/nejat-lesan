...
const MonthlyHolidayAnalyticsDashboard: React.FC<MonthlyHolidayAnalyticsDashboardProps> = ({
  ...
  const totalHolidayAccidents =
    data.series.find((s) => s.name === "تعطیل")?.data.reduce((sum, val) => sum + val, 0) || 0;
  const totalNonHolidayAccidents =
    data.series.find((s) => s.name === "غیر تعطیل")?.data.reduce((sum, val) => sum + val, 0) || 0;
    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">{formatNumber(totalAccidents)}</p>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-red-900">{formatNumber(totalHolidayAccidents)}</p>
        </div>
        ...
        <div className="bg-green-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-green-900">{formatNumber(totalNonHolidayAccidents)}</p>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-blue-900">
            {Number(holidayPercentage).toLocaleString("fa-IR")}%
          </p>
        </div>
      </div>
      ...
      {/* Chart */}
      <div className="h-96">
        <Chart options={chartOptions} series={data.series} type="bar" height="100%" />
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-blue-700">
              {Number(holidayPercentage).toLocaleString("fa-IR")}% از کل تصادفات در روزهای تعطیل رخ
              داده‌اند که نشان‌دهنده
              {parseFloat(holidayPercentage) > 20 ? " نرخ بالای" : " نرخ متعارف"} تصادفات در این روزها
              است.
            </p>
          </div>
          <div className="bg-amber-50 rounded-lg p-4">
            ...
            <p className="text-amber-700">
              در ماه‌هایی با تصادفات بالا در روزهای تعطیل، تدابیر ویژه کنترل ترافیک و آگاهی‌رسانی ضروری
              است.
            </p>
          </div>
        </div>
