...
const MonthlyHolidayAnalyticsDashboard: React.FC<MonthlyHolidayAnalyticsDashboardProps> = ({
  data,
  isLoading
}) => {
  // Loading state
  ...
  const totalHolidayAccidents = data.series.find(s => s.name === 'Holiday')?.data.reduce((sum, val) => sum + val, 0) || 0
  const totalNonHolidayAccidents = data.series.find(s => s.name === 'Non-Holiday')?.data.reduce((sum, val) => sum + val, 0) || 0
    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-gray-900">{totalAccidents.toLocaleString('fa-IR')}</p>
        </div>
        ...
        <div className="bg-red-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-red-900">{totalHolidayAccidents.toLocaleString('fa-IR')}</p>
        </div>

        <div className="bg-green-50 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-3 h-3 bg-green-500 rounded-full"></div>
            <h3 className="text-sm font-medium text-green-700">روزهای غیرتعطیل</h3>
          </div>
          <p className="text-2xl font-bold text-green-900">{totalNonHolidayAccidents.toLocaleString('fa-IR')}</p>
        </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-blue-900">{holidayPercentage}%</p>
        </div>
      </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-3">تحلیل و بینش</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <p className="text-blue-700">
              {holidayPercentage}% از کل تصادفات در روزهای تعطیل رخ داده‌اند که نشان‌دهنده
              {parseFloat(holidayPercentage) > 20 ? ' نرخ بالای' : ' نرخ متعارف'} تصادفات در این روزها است.
            </p>
          </div>
          <div className="bg-amber-50 rounded-lg p-4">
            ...
            <p className="text-amber-700">
              در ماه‌هایی با تصادفات بالا در روزهای تعطیل، تدابیر ویژه کنترل ترافیک و آگاهی‌رسانی ضروری است.
            </p>
          </div>
        </div>
      </div>
