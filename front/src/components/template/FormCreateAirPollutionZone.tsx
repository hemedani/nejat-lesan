...
import { add } from "@/app/actions/air_pollution_zone/add";
...
export const AirPollutionZoneCreateSchema = z.object({
...
export type AirPollutionZoneFormCreateSchemaType = z.infer<
  typeof AirPollutionZoneCreateSchema
>;
...
export const FormCreateAirPollutionZone = ({
  token,
  lesanUrl,
}: {
  token?: string;
  ...
  } = useForm<AirPollutionZoneFormCreateSchemaType>({
    resolver: zodResolver(AirPollutionZoneCreateSchema),
    mode: "onChange",
    defaultValues: {
      name: "",
      cityId: "",
  ...
  // Form submission
  const onSubmit: SubmitHandler<AirPollutionZoneFormCreateSchemaType> = async (
    data,
  ) => {
    const createdZone = await add({
      name: data.name,
    ...
    if (createdZone.success) {
      ToastNotify("success", "منطقه آلودگی هوا با موفقیت ایجاد شد");
      router.replace("/admin/air-pollution-zone");
    } else {
      ToastNotify(
        "error",
        createdZone.body.message || "خطا در ایجاد منطقه آلودگی هوا",
      );
    }
  };
    ...
    <div className="p-8">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {drawnPolygon && (
            <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
              ...
              <div className="text-xs text-green-700 space-y-1">
                ...
                <p>• برای تغییر منطقه، دکمه &quot;حذف منطقه&quot; را فشار دهید</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end">
          ...
          >
            {isSubmitting ? "در حال ایجاد..." : "ایجاد منطقه آلودگی هوا"}
          </button>
        </div>
      </form>
    </div>
