    ...
    defaultValues: accidentToEdit
      ? {
        ...
        date_of_accident: accidentToEdit.date_of_accident
          ? new Date(accidentToEdit.date_of_accident).toISOString().split('T')[0]
          : undefined,
  ...
  const onSubmit = async (data: AccidentFormValues) => {
    ...
    try {
      if (accidentToEdit?._id) {
        ...
        if (result.success) {
          ToastNotify("success", `${translateModelNameToPersian(model)} با موفقیت بروزرسانی شد`);
          router.refresh();
          onClose();
        } else {
          ToastNotify("error", `خطا در بروزرسانی ${translateModelNameToPersian(model)}: ${result.body?.message || "خطای نامشخص"}`);
        }
      } else {
        // Create new accident
        ...
        if (result.success) {
          ...
          ToastNotify("error", `خطا در ایجاد ${translateModelNameToPersian(model)}: ${result.body?.message || "خطای نامشخص"}`);
        }
      }
    } catch (error) {
        ...
        <div className="p-6">
          <div className="flex justify-between items-center border-b pb-4 mb-6">
            <h2 className="text-xl font-bold text-gray-800">
              {accidentToEdit ? `ویرایش ${translateModelNameToPersian(model)}` : `ایجاد ${translateModelNameToPersian(model)} جدید`}
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Seri Number */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">شماره سری</label>
                <input
                  type="number"
                ...
                />
                {errors.seri && (
                  <p className="mt-1 text-sm text-red-600">{errors.seri.message}</p>
                )}
              </div>

              {/* Date of Accident */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">تاریخ تصادف</label>
                <input
                  type="date"
                ...
                {errors.date_of_accident && (
                  <p className="mt-1 text-sm text-red-600">{errors.date_of_accident.message}</p>
                )}
              </div>

              {/* Officer */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">افسر</label>
                <input
                  type="text"
                  {...register("officer")}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500"
                />
                {errors.officer && (
                  <p className="mt-1 text-sm text-red-600">{errors.officer.message}</p>
                )}
              </div>

              {/* Dead Count */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">تعداد فوتی‌ها</label>
                <input
                  type="number"
                ...
                />
                {errors.dead_count && (
                  <p className="mt-1 text-sm text-red-600">{errors.dead_count.message}</p>
                )}
              </div>

              {/* Injured Count */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">تعداد مجروحین</label>
                <input
                  type="number"
                ...
                {errors.injured_count && (
                  <p className="mt-1 text-sm text-red-600">{errors.injured_count.message}</p>
                )}
              </div>
              ...
              <div className="flex items-center">
                ...
                />
                <label htmlFor="has_witness" className="mr-2 block text-sm text-gray-700">
                  دارای شاهد
                </label>
              </div>
            </div>
