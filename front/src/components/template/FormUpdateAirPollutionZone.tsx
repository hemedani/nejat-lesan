...
import { update } from "@/app/actions/air_pollution_zone/update";
...
export const AirPollutionZoneUpdateSchema = z.object({
...
export type AirPollutionZoneFormUpdateSchemaType = z.infer<
  typeof AirPollutionZoneUpdateSchema
>;
...
interface AirPollutionZoneData {
  _id: string;
  name: string;
  population: number;
  area: {
...
export const FormUpdateAirPollutionZone = ({
  token,
  lesanUrl,
  airPollutionZoneData,
  onSuccessAction,
}: {
  token?: string;
  lesanUrl: string;
  airPollutionZoneData?: AirPollutionZoneData;
  ...
  } = useForm<AirPollutionZoneFormUpdateSchemaType>({
    resolver: zodResolver(AirPollutionZoneUpdateSchema),
    mode: "onChange",
    defaultValues: {
      name: airPollutionZoneData?.name || "",
      population: airPollutionZoneData?.population || 0,
      area: airPollutionZoneData?.area || { type: "MultiPolygon", coordinates: [] },
    },
  });

  // Initialize map data from existing data
  useEffect(() => {
    if (airPollutionZoneData) {
      // Set form values
      setValue("name", airPollutionZoneData.name || "");
      setValue("population", airPollutionZoneData.population || 0);
      setValue(
        "area",
        airPollutionZoneData.area || { type: "MultiPolygon", coordinates: [] },
      );

      // Set map polygon
      if (
        airPollutionZoneData.area?.coordinates &&
        Array.isArray(airPollutionZoneData.area.coordinates) &&
        airPollutionZoneData.area.coordinates.length > 0
      ) {
        const firstPolygon = airPollutionZoneData.area.coordinates[0];
  ...
  }, [airPollutionZoneData, setValue, trigger]);
  ...
  // Form submission
  const onSubmit: SubmitHandler<AirPollutionZoneFormUpdateSchemaType> = async (
    data,
  ) => {
    if (!airPollutionZoneData?._id) {
      ToastNotify("error", "خطا: شناسه منطقه آلودگی هوا موجود نیست");
      return;
    }

    const updatedZone = await update(
      airPollutionZoneData._id,
      data.name,
      data.population,
      data.area as {
        type: "MultiPolygon";
    ...
    if (updatedZone.success) {
      ToastNotify("success", "منطقه آلودگی هوا با موفقیت ویرایش شد");
      onSuccessAction();
    } else {
      ToastNotify(
        "error",
        updatedZone.body.message || "خطا در ویرایش منطقه آلودگی هوا",
      );
    }
  };
      ...
      <div className="p-8">
        <div className="flex items-center justify-center py-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-gray-600">در حال بارگذاری اطلاعات منطقه آلودگی هوا...</p>
          </div>
        </div>
      </div>
    );
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="bg-gray-100 p-6 border rounded-lg space-y-4">
          <h2 className="text-lg font-semibold text-gray-700 mb-4">
            ویرایش اطلاعات منطقه آلودگی هوا
          </h2>
        ...
        <div className="bg-gray-100 p-6 border rounded-lg">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-700">
              ویرایش منطقه آلودگی هوا بر روی نقشه
            </h2>
        ...
        <div className="flex justify-end">
          ...
          >
            {isSubmitting ? "در حال ویرایش..." : "ویرایش منطقه آلودگی هوا"}
          </button>
        </div>
      </form>
    </div>
