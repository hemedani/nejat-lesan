...
const reviewStatuses = ["submitted", "under_review", "returned", "approved", "completed"];
...
const countReports = async (base: Record<string, unknown>) => {
	const sync = Object.fromEntries(await Promise.all(
		syncStatuses.map(async (status) => [
			status,
			await accident.countDocument({ filter: { ...base, sync_status: status } }),
		]),
	));
	const review = Object.fromEntries(await Promise.all(
...
}).sort({ reported_at: -1, _id: -1 }).skip((page - 1) * limit).limit(limit).toArray();

export const getReporterDashboardFn: ActFn = async (body) => {
	const context = coreApp.contextFns.getContextModel() as MyContext;
	if (context.user.level !== "Patrol") return throwError("این داشبورد فقط برای مأمور گشت است");
	const set = getSet(body);
	const get = body.details.get;
	const base = getReportScope(context.user);
	const activeShift = await shift.findOne({
		filters: { "officer._id": new ObjectId(context.user._id), status: "active" },
		projection: { _id: 1, shift_type: 1, status: 1, start_at: 1, end_at: 1, patrol_unit: 1, vehicle: 1 },
	});
	return {
		activeShift,
		summary: await countReports(base),
		recentReports: await recentReports(base, get, set.page || 1, Math.min(set.limit || 10, 50)),
	};
};
...
export const getManagerDashboardFn: ActFn = async (body) => {
	...
	return {
		summary: await countReports(base),
		recentReports: await recentReports(base, body.details.get as any, set.page || 1, Math.min(set.limit || 20, 100)),
	};
};
...
export const getManagerReportsFn: ActFn = async (body) => {
	...
	return await recentReports(filters, body.details.get as any, set.page || 1, Math.min(set.limit || 50, 100));
};
