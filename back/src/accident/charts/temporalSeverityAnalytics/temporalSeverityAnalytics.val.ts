/**
 * -----------------------------------------------------------------------------
 * FILE: temporalSeverityAnalytics.val.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalSeverityAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of **fatal vs. injury accidents** over time
 *
 ...
 * returns a single time-series of monthly fatality shares.
 */
import { array, enums, number, object, optional, string } from "@deps";
import { geoJSONStruct } from "@model";

export const temporalSeverityAnalyticsValidator = () => {
	return object({
		set: object({
		...
		get: object({
			analytics: enums([1]), // Always returns monthly fatality share
		}),
	});
};
