 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalTotalReasonAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of **top 10 ultimate causes** (`driver.total_reason.name`)
 *
 ...
 *
 * ⚠️ **Critical**: This act analyzes `vehicle_dtos.driver.total_reason.name`.
 *
 * The `get` section is minimal (`analytics: 1`) because this endpoint always
 * returns a multi-series time-series for the top 10 reasons.
 */
...
export const temporalTotalReasonAnalyticsValidator = () => {
	return object({
		...
		get: object({
			analytics: enums([1]), // Always returns top 10 reasons time-series
		}),
	});
};
