/**
 * -----------------------------------------------------------------------------
 * FILE: spatialSeverityAnalytics.fn.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Returns **dual analytics** for spatial severity comparison **by city zone**:
 * 1. **Stacked bar chart**: Counts of "فوتی", "جرحی", "خسارتی" per zone
 * 2. **Zonal map**: % of fatal accidents among severe ones (فوتی+جرحی) per zone
 *
...
export const spatialSeverityAnalyticsFn: ActFn = async (body) => {
	...
	//    - Bar chart: all severities
	//    - Map chart: % fatal among severe (فوتی+جرحی)
	// =========================================================================
	const pipeline: Document[] = [
		...
		{
			$facet: {
				barChartData: [ // Ensure type.name exists and is not null
					{
						$match: {
							"type.name": { $exists: true, $nin: [null, ""] },
						},
					},
					{
						$group: {
	...
	const fatalData = categories.map((cat: string) =>
		data.barChartData.find((d: any) => d.name === cat)?.counts["فوتی"] || 0
	);
	const injuryData = categories.map((cat: string) =>
		data.barChartData.find((d: any) => d.name === cat)?.counts["جرحی"] || 0
	);
	const damageData = categories.map((cat: string) =>
		data.barChartData.find((d: any) => d.name === cat)?.counts["خسارتی"] ||
		0
	);
	...
	return {
		analytics: {
			barChart: {
				...
				series: [
					{ name: "فوتی", fatalData },
					{ name: "جرحی", injuryData },
					{ name: "خسارتی", damageData },
				],
			},
