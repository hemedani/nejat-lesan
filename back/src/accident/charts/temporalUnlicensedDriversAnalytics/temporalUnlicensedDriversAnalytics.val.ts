/**
 * -----------------------------------------------------------------------------
 * FILE: temporalUnlicensedDriversAnalytics.val.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalUnlicensedDriversAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of accidents involving **unlicensed drivers**
 *
 ...
 *
 * ⚠️ **Critical**: This act **only analyzes accidents with at least one driver**
 * whose `licence_type.name` is **"فاقد گواهینامه"**.
 *
...
export const temporalUnlicensedDriversAnalyticsValidator = () => {
	return object({
		set: object({
		...
		get: object({
			analytics: enums([1]), // Always returns monthly unlicensed driver accident count
		}),
	});
};
