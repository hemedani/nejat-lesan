/**
 * -----------------------------------------------------------------------------
 * FILE: temporalCollisionAnalytics.val.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalCollisionAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of **selected collision types** over time
 *
 ...
 *
 * ⚠️ **Critical**: The `collisionType` filter defines which collision types to track.
 * If none provided, a **default set** is used.
 *
 * The `get` section is minimal (`analytics: 1`) because this endpoint always
 * returns a single time-series of monthly collision shares.
 */
import { array, enums, number, object, optional, string } from "@deps";
import { geoJSONStruct } from "@model";

export const temporalCollisionAnalyticsValidator = () => {
	return object({
		set: object({
			...
			collisionType: optional(array(string())), // ← CRITICAL: user-selected collision types
		...
		get: object({
			analytics: enums([1]), // Always returns monthly collision share
		}),
	});
};
