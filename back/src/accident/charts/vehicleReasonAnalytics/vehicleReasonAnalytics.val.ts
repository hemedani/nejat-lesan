/**
 * -----------------------------------------------------------------------------
 * FILE: vehicleReasonAnalytics.val.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "vehicleReasonAnalytics" act.
 *
...
export const vehicleReasonAnalyticsValidator = () => {
	return object({
		set: object({
			...
			vehicleReasons: optional(array(string())), // ← main focus of this chart
		...
		get: object({
			analytics: enums([1]), // Always returns both pie and bar chart data
		}),
	});
};
