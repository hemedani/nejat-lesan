 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "companyPerformanceAnalytics" act.
 *
 ...
 * - Deep, contextual analysis of vehicle manufacturer performance in **severe accidents only**
 *
 ...
 * returns bubble chart data for top manufacturers.
 */
...
export const companyPerformanceAnalyticsValidator = () => {
	return object({
		set: object({
			...
			vehicleSystem: optional(array(string())), // ← main grouping field (manufacturer)
		...
		get: object({
			analytics: enums([1]), // Always returns bubble chart data
		}),
	});
};
