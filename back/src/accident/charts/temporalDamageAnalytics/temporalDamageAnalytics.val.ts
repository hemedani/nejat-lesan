 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalDamageAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of vehicle damage patterns
 *
 ...
 *
 * The critical filter for this chart is `maxDamageSections` — it defines which
 * damage types to track over time.
 *
...
export const temporalDamageAnalyticsValidator = () => {
	return object({
		...
		get: object({
			analytics: enums([1]), // Always returns full analytics
		}),
	});
};
