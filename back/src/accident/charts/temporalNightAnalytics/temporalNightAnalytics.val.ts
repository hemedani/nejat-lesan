/**
 * -----------------------------------------------------------------------------
 * FILE: temporalNightAnalytics.val.ts
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "temporalNightAnalytics" act.
 *
 ...
 * - Flexible time-series analysis of **nighttime accidents** by lighting condition
 *
 ...
 *
 * ⚠️ **Critical**: This act **only analyzes nighttime accidents** with lighting conditions:
 *   - "شب با روشنایی کافی"
 *   - "شب با نور ناکافی"
 *
 * The `get` section is minimal (`analytics: 1`) because this endpoint always
 * returns a dual-series time-series.
 */
import { array, enums, number, object, optional, string } from "@deps";
import { geoJSONStruct } from "@model";

export const temporalNightAnalyticsValidator = () => {
	return object({
		set: object({
			...
			lightStatus: optional(array(string())), // ← but hardcoded to 2 values in pipeline
		...
		get: object({
			analytics: enums([1]), // Always returns dual-series time-series
		}),
	});
};
