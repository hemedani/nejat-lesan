 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "spatialSafetyIndexAnalytics" act.
 *
 ...
 * - Flexible spatial safety analysis by **province**, **city**, or **city_zone**
 *
 ...
 *
 * ⚠️ **Critical**: The `groupBy` field controls the **spatial unit**:
 *   - "province"
 *   - "city"
 *   - "city_zone"
 *
 * The `get` section is minimal (`analytics: 1`) because this endpoint always
 * returns a list of safety index metrics.
 */
...
export const spatialSafetyIndexAnalyticsValidator = () => {
	return object({
		...
		get: object({
			analytics: enums([1]), // Always returns safety index data
		}),
	});
};
