 ...
 * -----------------------------------------------------------------------------
 * DESCRIPTION:
 * Validator for the "humanReasonAnalytics" act.
 *
 ...
 * - Deep, contextual analysis of human factors (`human_reasons`) in accidents
 *
 ...
 *
 * ⚠️ **Critical**: The pipeline **excludes "ندارد"** (None) from results.
 *
 * The `get` section is minimal (`analytics: 1`) because this endpoint always
 * returns the top 8 human reasons.
 */
...
export const humanReasonAnalyticsValidator = () => {
	return object({
		set: object({
			...
			humanReasons: optional(array(string())), // ← main focus of this chart
		...
		get: object({
			analytics: enums([1]), // Always returns top 8 human reasons
		}),
	});
};
