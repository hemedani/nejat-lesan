...
export const seedFn: ActFn = async () => {
	...
	const normalizeRelations = async (
		...
		if (!item || !item.name || typeof item.name !== 'string' || item.name.trim().length === 0) return null;
		try {
			return await findOrCreate(item.name, modelName, userId);
		} catch (error) {
			console.error(`Failed to normalize relation ${modelName} with name ${item.name}:`, error);
			return null;
		}
	};
	...
	const validateAccidentData = (accident: AccidentJson): { isValid: boolean; errors: string[] } => {
		const errors: string[] = [];
	...
	const processAccidentRecord = async (element: { accident_json: string }) => {
		try {
			...
			if (!validation.isValid) {
				...
				console.log(`Skipping accident with serial ${parsedAccident.serialNO || 'unknown'} due to validation errors:`, validation.errors);
				return;
			}
			...
			officer: parsedAccident.officer ? normalizePersianText(parsedAccident.officer) : '',
		...
		if (parsedAccident.province) {
			relationPromises.push(
				normalizeRelations(parsedAccident.province, "province", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.township) {
			relationPromises.push(
				normalizeRelations(parsedAccident.township, "city", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.road) {
			relationPromises.push(
				normalizeRelations(parsedAccident.road, "road", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.type) {
			relationPromises.push(
				normalizeRelations(parsedAccident.type, "type", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.position) {
			relationPromises.push(
				normalizeRelations(parsedAccident.position, "position", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.rulingType) {
			relationPromises.push(
				normalizeRelations(parsedAccident.rulingType, "ruling_type", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.lightStatus) {
			relationPromises.push(
				normalizeRelations(parsedAccident.lightStatus, "light_status", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.collisionType) {
			relationPromises.push(
				normalizeRelations(parsedAccident.collisionType, "collision_type", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.roadSituation) {
			relationPromises.push(
				normalizeRelations(parsedAccident.roadSituation, "road_situation", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.roadRepairType) {
			relationPromises.push(
				normalizeRelations(parsedAccident.roadRepairType, "road_repair_type", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.shoulderStatus) {
			relationPromises.push(
				normalizeRelations(parsedAccident.shoulderStatus, "shoulder_status", userId).then(normalized => {
					if (normalized) {
		...
		if (parsedAccident.areaUsages) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.areaUsages, "area_usage", userId).then(normalized => {
		...
		if (parsedAccident.airStatuses) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.airStatuses, "air_status", userId).then(normalized => {
		...
		if (parsedAccident.roadDefects) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.roadDefects, "road_defect", userId).then(normalized => {
		...
		if (parsedAccident.humanReasons) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.humanReasons, "human_reason", userId).then(normalized => {
		...
		if (parsedAccident.vehicleReasons) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.vehicleReasons, "vehicle_reason", userId).then(normalized => {
		...
		if (parsedAccident.equipmentDamages) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.equipmentDamages, "equipment_damage", userId).then(normalized => {
		...
		if (parsedAccident.roadSurfaceConditions) {
			relationPromises.push(
				normalizeArrayRelations(parsedAccident.roadSurfaceConditions, "road_surface_condition", userId).then(normalized => {
			...
			if (parsedAccident.vehicleDTOS?.length) {
				...
				parsedAccident.vehicleDTOS.filter(v => v && v.driver).map(async (v) => {
					// Basic validation for vehicle data
					if (!v.driver) {
						console.warn(`Skipping vehicle with no driver data for accident ${parsedAccident.serialNO || 'unknown'}`);
						return null;
					}
					...
					try {
						return {
							...
							body_insurance_date: new Date(v.bodyInsuranceDate),
							insurance_warranty_limit: v.insuranceWarrantyLimit || 0,
						...
						driver: {
							first_name: v.driver.firstName ? normalizePersianText(v.driver.firstName) : '',
							last_name: v.driver.lastName ? normalizePersianText(v.driver.lastName) : '',
							national_code: v.driver.nationalCode ? normalizePersianText(
							...
							sex: v.driver.sex ? (
								v.driver.sex.name === "مرد" ? "Male" :
								v.driver.sex.name === "زن" ? "Female" : "Other"
							) : "Other",
						...
						passenger_dtos: v.passengerDTOS?.length
							? await Promise.all(
								v.passengerDTOS.map(async (p) => ({
									first_name: normalizePersianText(
										p.firstName,
									),
									last_name: normalizePersianText(p.lastName),
									national_code: normalizePersianText(
										p.nationalCode,
									),
									sex: p.sex ? (
										p.sex.name === "مرد" ? "Male" :
										p.sex.name === "زن" ? "Female" : "Other"
									) : "Other",
									injury_type: await normalizeRelations(
										p.injuryType,
										"max_damage_section",
						...
						console.error(`Error processing vehicle data for accident ${parsedAccident.serialNO || 'unknown'}:`, error);
						return null;
					}
				})
			);
			accidentDoc.vehicle_dtos = vehicleResults.filter(v => v !== null) as any;
			}

			// Optional: handle pedestrianDTOS
			if (parsedAccident.pedestrianDTOS?.length) {
				const pedestrianResults = await Promise.all(
				parsedAccident.pedestrianDTOS.filter(p => p && p.firstName && p.lastName).map(async (p) => ({
			...
			try {
				...
				console.error(`Failed to insert accident with serial ${parsedAccident.serialNO || 'unknown'}:`, error);
	...
	console.log(`Coordinate validation success rate: ${((validCoordinatesCount / totalRecords) * 100).toFixed(1)}%`);
	console.log(`Total time: ${(totalTime / 1000).toFixed(2)} seconds`);
	console.log(`Average time per record: ${avgTimePerRecord.toFixed(2)}ms`);
	console.log(`Records per second: ${(processedCount / (totalTime / 1000)).toFixed(2)}`);
	console.log(`=========================\n`);
