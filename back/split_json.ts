...
async function splitLargeJsonFile(
  ...
  const readNextChar = async (): Promise<string | null> => {
    ...
    if (bufferIndex + charLength > rawData.length) {
      ...
      if (bytesRead === null) {
        ...
        throw new Error("Unexpected end of file in the middle of UTF-8 character");
      }
    ...
    const charBytes = rawData.subarray(bufferIndex, bufferIndex + charLength);
    ...
    if (!inString) {
      if (char === '{') {
        ...
        if (braceCount === 0) {
          ...
          const objSize = new TextEncoder().encode(currentObject).length;
          const estimatedNewSize = currentChunkSize + objSize +
                                  (currentChunk.length > 0 ? 2 : 1); // comma + space, or just opening bracket

          if (currentChunk.length === 0 || estimatedNewSize <= maxChunkSize) {
            ...
            await writeChunkToFile(currentChunk, chunkIndex, outputDir, outputPrefix);
            console.log(`Chunk ${chunkIndex} completed with ${currentChunk.length} objects`);
  ...
  if (currentChunk.length > 0) {
    await writeChunkToFile(currentChunk, chunkIndex, outputDir, outputPrefix);
    console.log(`Final chunk ${chunkIndex} completed with ${currentChunk.length} objects`);
  }
...
async function writeChunkToFile(
  ...
  const content = "[\n" + chunk.join(",\n") + "\n]";
  const outputFile = `${outputDir}/${outputPrefix}_part_${index.toString().padStart(3, '0')}.json`;
